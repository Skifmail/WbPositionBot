...
class Article(Base):
	...
	id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
	user_id: Mapped[int] = mapped_column(ForeignKey("wbpos.users.id", ondelete="CASCADE"))
	sku: Mapped[int] = mapped_column(index=True)
	title: Mapped[Optional[str]] = mapped_column(String(256), nullable=True)
	created_at: Mapped[datetime] = mapped_column(default=datetime.utcnow)

	user: Mapped[User] = relationship(back_populates="articles")
	trackings: Mapped[list[Tracking]] = relationship(back_populates="article", cascade="all, delete-orphan")
...
class Tracking(Base):
	...
	id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
	article_id: Mapped[int] = mapped_column(ForeignKey("wbpos.articles.id", ondelete="CASCADE"))
